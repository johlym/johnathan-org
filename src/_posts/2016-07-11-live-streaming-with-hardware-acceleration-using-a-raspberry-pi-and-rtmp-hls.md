...
So far we've tried `ffmpeg` => `ffserver` and that [turned out](/fighting-ffmpeg/) exactly how you probably thought it would. Next, I tried [swapping `ffserver`](/attempting-to-stream-a-webcam-to-an-rtmp-server/) with an Nginx-powered RTMP server. While not an entirely fruitless endeavor, there were [some blockages](/attempting-to-stream-a-webcam-to-an-rtmp-server/) that I just couldn't get past.
...
Since I felt like including it here would deviate from the purpose of this post too much, I wrote up how to [Add RTMP Support](/add-rtmp-support-to-nginx-installed-from-apt/) to Nginx if you installed it via `apt-get` like me. If you're in that boat, take a moment to read over that post then come back to this one.

Setting up `ffmpeg` to use hardware `H.264` encoding used to be a fat challenge, but they've since added support to the official codebase. If you followed my [original ffmpeg](/attempting-to-stream-a-webcam-to-an-rtmp-server/) post, you'll have a recent enough version that includes this code, but we'll still need to compile it.
...
From this point forward, we'll be starting `ffmpeg` similarly to how we did it [before](/attempting-to-stream-a-webcam-to-an-rtmp-server/) but with a slightly different codec.
